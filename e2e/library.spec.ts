...
test("saves a comparison in this browser and reopens it", async ({ page }) => {
  ...
  const name = page.locator(".account-dialog").getByLabel("Name", { exact: true });
  await expect(name).toHaveValue("second version");
  await name.fill("My test comparison");
  await page.locator(".account-dialog").getByRole("button", { name: "Save", exact: true }).click();
...
test("renames and deletes saved comparisons", async ({ page }) => {
  ...
  await page.locator(".account-dialog").getByLabel("Name", { exact: true }).fill("Renamed");
  await page.locator(".account-dialog").getByRole("button", { name: "Rename", exact: true }).click();
...
test("the sign-in dialog offers Google and an email link", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(async () => {
    await account.click();
    await expect(page.getByRole("button", { name: "Continue with Google" })).toBeVisible({ timeout: 1000 });
  }).toPass({ timeout: 10000 });
  await expect(page.getByLabel("Email address")).toBeVisible();
  await expect(page.locator(".account-dialog")).toContainText("encrypted in your browser");
});
